...
import { Search, Plus, Upload, Download, Edit2, Trash2, ChevronRight, Phone } from 'lucide-react';
...
export default function CustomersPage() {
  ...
  const handleExport = () => {
    const exportData = customers.map(c => {
      const custContracts = contracts.filter(ct => ct.customerId === c.id && ct.status === 'active');
      const balance = custContracts.reduce((sum, ct) => sum + ct.remainingAmount, 0);
      return {
        ID: c.id,
        Name: c.name,
        Phone: c.phone,
  ...
  const getCustomerStats = (customerId: string) => {
    const custContracts = contracts.filter(c => c.customerId === customerId && c.status === 'active');
    const balance = custContracts.reduce((sum, c) => sum + c.remainingAmount, 0);
    return { count: custContracts.length, balance };
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 mt-4">
        {filteredCustomers.map(customer => {
          ...
          return (
            <Card key={customer.id} className="relative !p-0 overflow-hidden group">
              <div className="p-4 cursor-pointer" onClick={() => navigate(`/customers/${customer.id}`)}>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 border-t border-gray-50 dark:border-gray-800/50 pt-3">
                  <div>
                    <div className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1">العقود النشطة</div>
                    <div className="font-medium text-gray-900 dark:text-gray-100">{stats.count}</div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1">إجمالي الرصيد</div>
                    <div className="font-medium text-rose-600 dark:text-rose-400">{formatCurrency(stats.balance)}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute top-4 right-4 flex items-center gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button 
                  onClick={(e) => { e.stopPropagation(); if(confirm('هل تريد حذف العميل؟')) deleteCustomer(customer.id); }} 
        ...
        {filteredCustomers.length === 0 && (
          <div className="text-center py-12 text-gray-400">لم يتم العثور على عملاء</div>
        )}
      </div>

      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'تعديل العميل' : 'إضافة عميل'}>
        <div className="space-y-4">
          <Input 
            label="الاسم الكامل" 
            value={formData.name} 
