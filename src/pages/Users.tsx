    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center h-[60vh]">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          عذراً، غير مصرح لك بالوصول لإدارة المستخدمين. هذه الصفحة مخصصة لمدير
          النظام فقط.
        </p>
      </div>
    );
    ...
    <div className="space-y-6 pb-6" dir="rtl">
      <div>
        ...
        <p className="text-sm text-slate-500">
          إضافة وتعديل صلاحيات الدخول للنظام
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredUsers.map((user) => (
          <Card
            key={user.id}
            className={`p-5 flex flex-col gap-4 border-slate-200/60 dark:border-slate-800 bg-white dark:bg-slate-950 transition-all hover:shadow-md ${!user.isActive ? "opacity-70" : ""}`}
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3
                    className={`font-bold ${user.isActive ? "text-slate-900 dark:text-slate-100" : "text-slate-500 line-through"}`}
                  >
                    {user.name}
                  </h3>
                  <div
              ...
              <div
                className={`text-xs font-bold px-2.5 py-1 rounded-full ${user.isActive ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20" : "bg-slate-100 text-slate-500 dark:bg-slate-800"}`}
              >
            ...
            {user.id !== currentUser.id && (
              <div className="flex items-center gap-2 mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 bg-slate-50 hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 p-2.5 rounded-xl font-medium text-xs transition-colors"
                >
          ...
          <Card className="w-full max-w-sm p-6 space-y-6 bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 shadow-2xl animate-in zoom-in-95 duration-200">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                يرجى تعبئة الحقول أدناه لحفظ الحساب.
              </p>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5 text-slate-700 dark:text-slate-300">
                  ...
                  {editingUserId && (
                    <span className="text-xs text-slate-400 font-normal mr-2">
                      (اتركه فارغاً للاحتفاظ بالحالية)
                    </span>
                  )}
                </label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder={
                      editingUserId ? "••••••••" : "أدخل كلمة مرور قوية"
                    }
                    ...
                    dir="ltr"
                    className="h-12 rounded-xl pr-10 pl-10 text-left"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors focus:outline-none"
                  >
                    {showPassword ? (
              ...
              <div className="flex gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                <button
                  ...
                  className="flex-1 bg-white hover:bg-slate-50 text-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl py-3 font-semibold transition-colors"
                >
                  إلغاء
                </button>
                <button
                  type="submit"
                  className="flex-[2] bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-3 font-bold shadow-sm shadow-indigo-600/20 transition-colors"
                >
                  حفظ التغييرات
                </button>
              </div>
